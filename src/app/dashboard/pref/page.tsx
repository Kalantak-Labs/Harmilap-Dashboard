...
export default async function Page({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const positions = filterPositions("PREFERENCE", { q, filter, f_company, f_class, f_holder, f_pan });
  const companyFilterOpts = companies.map((c) => ({ label: c.name, value: c.id }));
  const raw = filterPositions("PREFERENCE", {});
  ...
  return (
    <div className="flex-col gap-6">
      <header className="flex justify-between items-center mb-4">
        <div><h2>Preference Register</h2><p className="text-sub">Beneficiary position register.</p></div>
        <div className="flex gap-4 items-center">
          <DataTableFilter searchPlaceholder="Search..." filterOptions={companyFilterOpts} />
          <SecurityModals companies={companies} defaultType="PREFERENCE" />
        </div>
      </header>
